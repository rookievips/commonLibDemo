    ...
    private void reloadData() {
        ...
        for (int i = 0; i < mAdapter.getCount(); i++) {
            ...
            if (mAdapter instanceof OnInitSelectedPosition) {
                int isSelected = ((OnInitSelectedPosition) mAdapter).getSelectedPosition();

                //判断一下模式
                if (mTagCheckMode == FLOW_TAG_CHECKED_SINGLE) {
                    //单选只有第一个起作用
                    if (isSelected == i && !isSet) {
                    ...
                    if (mTagCheckMode == FLOW_TAG_CHECKED_NONE) {
                        ...
                        if (mCheckedTagArray.get(j)) {
                            ...
                            if (mOnTagSelectListener != null) {
                                mOnTagSelectListener.onItemSelect(FlowTagLayout.this, Arrays.asList(j), false);
                            }
                            return;
                        }
                        ...
                        if (mOnTagSelectListener != null) {
                            ...
                            mOnTagSelectListener.onItemSelect(FlowTagLayout.this, list, false);
                        }
                    }
                }
            });
